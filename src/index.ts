...
`
	...
	if (!globalWhopctl.version) {
		...
		console.log('')
	} else if (localWhopctlVersion && globalWhopctl.version.trim() !== localWhopctlVersion.trim()) {
		console.log(
			chalk.yellow(
		...
		console.log(
			chalk.dim(
				'   Update it with npm install -g @whoplabs/whopctl@latest or use the npm scripts above.',
			),
		)
		console.log('')
	}
